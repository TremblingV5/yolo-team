...
- 如果用户处于 **SDD 工作流**或**计划模式**下（即已存在类似任务拆解的具体文档），AI 应自行拆解目标，创建对应的 Issue 和 Task
- 否则，AI **必须先询问用户**是否需要进行任务拆解，并在获得确认后生成拆解文档，再执行后续操作
- 拆分任务时，应注重**合理拆分**，将可并行的任务拆解到不同的 Issue 中，提供并行执行的可能性

拆解过程分为多个大步骤：

#### 方案设计阶段

1. 给 **leader**（项目负责人）创建一个整体的 Issue，描述本 Issue 主要要做什么事。
   - Issue 的描述中写明：只有关联了**方案设计文档、代码实现报告、测试报告、代码审查报告**后，才可以将本 Issue 推进到已完成。
2. 给 **architect**（架构师）创建子 Issue，完成本 Issue 的方案设计。
   - 方案设计文档生成后，关联到整体的 Issue 中。
   - 方案设计需要遵循 **OpenSpec 规范**。

#### 功能实现阶段
按模块拆分 Issue 和 Task，推进编码实现。

#### 最终审查阶段
审查所有实现是否符合方案设计，输出审查报告。
